	  ...
	  public Node getFirst(){
	   ...
	   public boolean insertAfter(Object obj1, Object obj2){ 
			...
			while (current!=null) { 
				if (obj1.equals(current.getData())) { 
					...
					if ( current.getNext() != null ) { //se tiver um pr�ximo elemento 
						tmpNode.setNext(current.getNext()); 
				} else { //se for o ultimo elemento 
					lastNode = tmpNode; 
					tmpNode.setNext(null); 
				}  //ou utilizar o m�todo insertAtBack()
	   ...
	   public boolean remove(Object o) { 
			...
			if (firstNode.getData().equals(o)){ //se for o primeiro 
				removeFromFront(); 
				return true; 
			} else if (lastNode.getData().equals(o)){ //se for o �ltimo 
				removeFromBack(); 
				return true;
			} 
